...
class ModelClass:
    ...
    def __init__(self, configuration, load_from_file, quiet):
        self.conf = {key: configuration[key] if key in configuration else
        ModelClass.default_configuration[key] for key in
                     ModelClass.default_configuration.keys()}
        self.quiet = quiet
    ...
    def init_data_structures(self, file_suffix):
        ''' Initialize the records structures.
        ...
        if file_suffix == None:
            ...
            self.synapse_strength[:, excitatory_unit_num:] = (-1) * self.synapse_strength[
                                                                    :,
                                                                    excitatory_unit_num:]
        else:
            file_name = "synapse_strength"
        ...
        # Input to 1st layer
        '''Consider the N^2 input neurons as an NXN grid. Each neuron is connected only 
        to its
        nearest neighbors. '''
        ...
        for unit_ind in range(self.conf['layers_size'][0]):
            ...
            if unit_ind < self.conf['layers_size'][0] - N:
                self.zero_matrix[unit_ind, unit_ind + N] = 1
        self.zero_matrix[:self.conf['layers_size'][0],
        self.conf['layers_size'][0]:sum(self.conf['layers_size'])] = 0

        # Input to 1st IIN
        self.zero_matrix[sum(self.conf['layers_size']),
        self.conf['layers_size'][0]:sum(self.conf['layers_size'])] = 0
        ...
        for l in range(1, len(self.conf['layers_size'])):
            ...
            self.zero_matrix[
            sum(self.conf['layers_size'][:l]):sum(self.conf['layers_size'][:l + 1]),
            :sum(self.conf['layers_size'][:l - 1])] = 0
            self.zero_matrix[
            sum(self.conf['layers_size'][:l]):sum(self.conf['layers_size'][:l + 1]),
            sum(self.conf['layers_size'][:l]):sum(self.conf['layers_size'])] = 0

            # Input to IIN
            self.zero_matrix[sum(self.conf['layers_size']) + l,
            :sum(self.conf['layers_size'][:l])] = 0
            self.zero_matrix[sum(self.conf['layers_size']) + l,
            sum(self.conf['layers_size'][:l + 1]):sum(self.conf['layers_size'])] = 0

            # Input from IIN
            self.zero_matrix[:sum(self.conf['layers_size'][:l]),
            sum(self.conf['layers_size']) + l] = 0
            self.zero_matrix[
            sum(self.conf['layers_size'][:l + 1]):sum(self.conf['layers_size']),
            sum(self.conf['layers_size']) + l] = 0
            self.zero_matrix[sum(self.conf['layers_size']):,
            sum(self.conf['layers_size']) + l] = 0
    ...
    def init_normalization_parameters(self):
        ...
        For the following layers (except the last one) we want half of the neurons to
        be winners,
        to maximize the number of possible combinations in the next layers.
        In the last layer we want a single winner, because each input corresponds to a
        single class.
        '''
        ...
        self.conf['Z_iin'] = [x * self.conf['excitatory_threshold'] for x in
                              self.conf['Z_iin_ex_th_ratio']]
        self.conf['Z_intra_layer'] = self.conf['Z_intra_layer_ex_th_ratio'] * self.conf[
        ...
        for l in range(1, len(self.conf['layers_size'])):
            ...
            self.conf['Z_vals'].append(
                (self.conf['excitatory_threshold'] * self.conf['layers_size'][l - 1]) / (
                            norm_shrink_factor * self.conf['winner_num'][l - 1]))

        self.conf['Z_ex_to_iins'] = []
        for l in range(len(self.conf['layers_size'])):
            self.conf['Z_ex_to_iins'].append(
                (self.conf['excitatory_threshold'] * self.conf['layers_size'][l]) / (
                self.conf['winner_num'][l]))


    def reset_data_structures(self):
        self.prev_act = np.zeros(
            (sum(self.conf['layers_size']) + len(self.conf['layers_size']), 1))
        self.prev_input_to_neurons = np.zeros(
            (sum(self.conf['layers_size']) + len(self.conf['layers_size']), 1))
    ...
    def fix_synapse_strength(self):
        ...
        # Also make sure intra-layer connections for input neurons are all of equal
        # strength
        self.synapse_strength[:self.conf['layers_size'][0],
        :self.conf['layers_size'][0]] = 1
        self.synapse_strength[
        self.conf['layers_size'][0]:sum(self.conf['layers_size'][:1]),
        self.conf['layers_size'][0]:sum(self.conf['layers_size'][:1])] = 1

        # Enforce invariants
        self.synapse_strength = np.multiply(self.synapse_strength, self.zero_matrix)

        # Make sure excitatory weights are all excitatory
        self.synapse_strength[:, :excitatory_unit_num][
            self.synapse_strength[:, :excitatory_unit_num] < 0] = 0

        # Normalize incoming excitatory weights to each unit    

        # Input to 1st layer
        first_input_to_first_input_row_sum = (self.synapse_strength[
                                              :self.conf['layers_size'][0],
                                              :self.conf['layers_size'][0]].sum(
            axis=1).reshape(self.conf['layers_size'][0], 1).repeat(
            self.conf['layers_size'][0], axis=1)) / self.conf['Z_intra_layer']
        rest_to_first_input_row_sum = np.ones((self.conf['layers_size'][0],
                                               sum(self.conf['layers_size']) -
                                               self.conf['layers_size'][0]))
        first_input_row_sum = np.concatenate(
            (first_input_to_first_input_row_sum, rest_to_first_input_row_sum), axis=1)

        excitatory_row_sums = first_input_row_sum

        for l in range(1, len(self.conf['layers_size'])):
            # Input to EINs
            input_from_earlier_layers = np.ones(
                (self.conf['layers_size'][l], sum(self.conf['layers_size'][:l - 1])))
            input_from_prev_layer = (self.synapse_strength[
                                     sum(self.conf['layers_size'][:l]):sum(
                                         self.conf['layers_size'][:l + 1]),
                                     sum(self.conf['layers_size'][:l - 1]):sum(
                                         self.conf['layers_size'][:l])].sum(
                axis=1).reshape(self.conf['layers_size'][l], 1).repeat(
                self.conf['layers_size'][l - 1], axis=1)) / self.conf['Z_vals'][l - 1]
            input_from_later_layers = np.ones(
                (self.conf['layers_size'][l], sum(self.conf['layers_size'][l:])))
            row_sum = np.concatenate((input_from_earlier_layers, input_from_prev_layer,
                                      input_from_later_layers), axis=1)
        ...
        for l in range(len(self.conf['layers_size'])):
            ...
            input_from_layer = (self.synapse_strength[sum(self.conf['layers_size']) + l,
                                sum(self.conf['layers_size'][:l]):sum(
                                    self.conf['layers_size'][:l + 1])].sum().repeat(
                self.conf['layers_size'][l]).reshape(1, self.conf['layers_size'][l])) / \
                               self.conf['Z_ex_to_iins'][l]
            input_from_later_layers = np.ones((1, sum(self.conf['layers_size'][l + 1:])))
            row_sum = np.concatenate(
                (input_from_earlier_layers, input_from_layer, input_from_later_layers),
                axis=1)
            excitatory_row_sums = np.concatenate((excitatory_row_sums, row_sum), axis=0)
        ...
        for l in range(len(self.conf['layers_size'])):
            cur_layer_inhibitory_row_sums = (-1) * ((self.synapse_strength[:,
                                                     [excitatory_unit_num + l]].sum(
                axis=1).reshape(unit_num, 1)) / self.conf['Z_iin'][l])
            inhibitory_row_sums = np.concatenate(
                (inhibitory_row_sums, cur_layer_inhibitory_row_sums), axis=1)
    ...
    def update_synapse_strength(self, cur_fire_count):
        for l in range(1, len(self.conf['layers_size'])):
            prev_layer_winners = np.array([[1 if cur_fire_count[sum(
                self.conf['layers_size'][:l - 1]) + i] >= 0.5 * cur_fire_count[sum(
                self.conf['layers_size']) + l - 1] else 0 for i in range(
                self.conf['layers_size'][l - 1])]]).transpose()
            cur_layer_winners = np.array([[1 if cur_fire_count[sum(
                self.conf['layers_size'][:l]) + i] >= 0.5 * cur_fire_count[sum(
                self.conf['layers_size']) + l] else 0 for i in range(
                self.conf['layers_size'][l])]]).transpose()
            update_mat = np.matmul(2 * (cur_layer_winners - 0.5),
                                   prev_layer_winners.transpose())
            self.synapse_strength[
            sum(self.conf['layers_size'][:l]):sum(self.conf['layers_size'][:l + 1]),
            sum(self.conf['layers_size'][:l - 1]):sum(
                self.conf['layers_size'][:l])] += update_mat * self.conf['eta'] * (
                        self.conf['Z_vals'][l - 1] / self.conf['layers_size'][l - 1])
    ...
    def simulate_dynamics(self, input_vec, innervated_response_neuron):
        ...
        sensory_input_vec = np.pad(input_vec,
                                   ((0, unit_num - self.conf['layers_size'][0]), (0, 0)),
                                   'constant')
        if innervated_response_neuron != -1:
            sensory_input_vec[
                sum(self.conf['layers_size'][:-1]) + innervated_response_neuron, 0] = \
            self.conf['response_innervation_strength']
    ...
    def prop_external_input(self, sensory_input_vec):
        ...
        cur_act = np.concatenate((self.excitatory_activation_function(
            cur_input[:sum(self.conf['layers_size']), [0]]),
                                  self.inhibitory_activation_function(
                                      cur_input[sum(self.conf['layers_size']):, [0]])),
                                 axis=0)
...
training_set = generate_training_set_no_generalization(
    ModelClass.default_configuration['sensory_input_strength'])
...
def plot_precision_as_a_func_of_training_epoch_num():
    ...
    for cur_training_epoch in range(training_epoch_num):
        ...
        cur_time = time.time()
        log_print('\t\tEpoch ' + str(cur_training_epoch) + ', prev epoch took ' + str(
            prev_epoch_time))
        ...
        for input_ind in range(len(test_set)):
            # log_print('\t\t\tEvaluating input ' + str(input_ind) + ' of ' + str(len(
            # test_set)))
            ...
            correct = len([x for x in fire_count[sum(model.conf['layers_size'][:-1]):sum(
                model.conf['layers_size'])] if x > 0]) == 1 and \
